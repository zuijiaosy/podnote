...
Podnote:本地单机的播客笔记仪器(macOS/Tauri)。小宇宙单集链接 → 云端转写(阿里百炼 fun-asr,含说话人分离)→ LLM 生成带归属的结构化笔记。产品哲学:自带 API 钥匙(BYO-key)、数据全在本机、每次花钱的动作由用户显式触发且成本透明、笔记是收听的伴生品(时间戳回跳重听)而非替代品。

仓库有两代形态:
- `app/` — 现行 Tauri 桌面应用(React 前端 + Rust 后端),主战场
- 根目录 `src/index.mjs` + `data/` + `notes/` — 初代 Node MVP 管线(`npm run note <url>`),已被 Tauri 版取代,保留作参考
...
# 产物: app/src-tauri/target/release/bundle/dmg/Podnote_*.dmg
```

**交付约定:每轮代码修改完成后必须重新打包 dmg 并告知产物路径**——用户不跑 dev 模式,靠安装 dmg 测试。打包耗时长,用后台任务跑。
...
### 密钥与设置
- 密钥存 app 数据目录 `keys.json` 明文(自用取舍:无签名证书时钥匙串每次打包都重复弹窗),启动读一次进内存,运行期零钥匙串访问
...
`core.jsx`(Button/Input/Select/Segmented/Lever/Checkbox/FieldRow) + `instrument.jsx`(StatusLabel/IndicatorLight/Timestamp/EpisodeItem/Waveform),移植自 `Podnote 正式设计 standalone.html`(设计真源)。仪器风硬规则:
